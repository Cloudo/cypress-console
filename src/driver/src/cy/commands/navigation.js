...
const cannotVisitDifferentOrigin = (
  origin,
  previousUrlVisited,
  remoteUrl,
  existingUrl,
  ...
  Commands.addAll({
    ...
    visit(url, options = {}) {
      ...
      const go = () => {
        ...
        if (
          previousDomainVisited &&
          remote.originPolicy !== existing.originPolicy
        ) {
          // if we've already visited a new superDomain
          // then die else we'd be in a terrible endless loop
          return cannotVisitDifferentOrigin(
            remote.origin,
            previousDomainVisited,
            remote,
            existing,
