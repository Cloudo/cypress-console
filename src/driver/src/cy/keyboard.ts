...
const shouldUpdateValue = (
  ...
  if ($elements.isInput(el) || $elements.isTextarea(el)) {
    ...
    const isNumberInputType =
      $elements.isInput(el) && $elements.isInputType(el, 'number')
    ...
    if (isNumberInputType) {
      ...
      const potentialValue = $selection.insertSubstring(
        curVal + needsValue,
        key.text,
        [bounds.start + needsValueLength, bounds.end + needsValueLength]
      )

      if (!numberRe.test(potentialValue)) {
        debug(
          'skipping inserting value since number input would be invalid',
          key.text,
          potentialValue
        )
...
const validateTyping = (
  ...
  if ($elements.isInput(el)) {
    ...
    isDateTime =
      $elements.isAttrType(el, 'datetime') ||
      $elements.isAttrType(el, 'datetime-local')
  }
...
export class Keyboard {
  ...
  type(opts: typeOptions) {
      ...
      (key: KeyDetails, currentKeyIndex: number) => {
        return () => {
          ...
          _skipCheckUntilIndex =
            _skipCheckUntilIndex && _skipCheckUntilIndex - 1
          ...
          if (!_skipCheckUntilIndex) {
            ...
            if (
              ...
              const keysToType = keyDetailsArr.slice(
                currentKeyIndex,
                currentKeyIndex + _skipCheckUntilIndex
              )
              ...
              _.last(keysToType)!.simulatedDefault = () => {
                ...
                const valToSet = isClearChars
                  ? ''
                  : joinKeyArrayToString(keysToType)
            ...
            debug(
              'skipping validation due to *skipCheckUntilIndex*',
              _skipCheckUntilIndex
            )
          }
    ...
    const modifierKeys = _.chain(keyDetailsArr)
      .filter(isModifier)
      .uniqBy('key')
      .value()
  ...
  getActiveModifiers() {
    return (
      _.clone(this.state('keyboardModifiers')) || _.clone(INITIAL_MODIFIERS)
    )
  }
