...
const create = function (specWindow, Cypress, Cookies, state, config, log) {
  ...
  const urlNavigationEvent = (event) => {
    return Cypress.action(
      'app:navigation:changed',
      `page navigation event (${event})`
    )
  }
  ...
  const wrapNativeMethods = function (contentWindow) {
    try {
      ...
      contentWindow.CSSStyleSheet.prototype.insertRule = _.wrap(
        insertRule,
        cssModificationSpy
      )
      contentWindow.CSSStyleSheet.prototype.deleteRule = _.wrap(
        deleteRule,
        cssModificationSpy
      )
  ...
  const getCommandsUntilFirstParentOrValidSubject = function (
    command,
    memo = []
  ) {
    if (!command) {
    ...
    if (
      ...
      $dom.isAttached(command.get('subject'))
    ) {
      return memo
    }
  ...
  const pushSubjectAndValidate = function (name, args, firstCall, prevSubject) {
    if (firstCall) {
      ...
      if (
        prevSubject &&
        ((needle = 'optional'), ![].concat(prevSubject).includes(needle))
      ) {
        const stringifiedArg = $utils.stringifyActual(args[0])

        $errUtils.throwErrByPath(
          'miscellaneous.invoking_child_without_parent',
          {
            args: {
              cmd: name,
              args: _.isString(args[0])
                ? `\"${stringifiedArg}\"`
                : stringifiedArg,
            },
          }
        ...
        const commandsToInsert = _.reduce(
          ...
          (memo, command, index) => {
            ...
            if (!(command.get('type') !== 'assertion')) {
              ...
              !(command.get('subject') === initialCommand.get('subject'))
            ) {
      ...
      runnable.fn = function () {
        ...
        try {
          ...
          if (
            ...
            $errUtils.throwErrByPath(
              'miscellaneous.returned_value_and_commands',
              {
                args: { returned: ret },
              }
            )
