...
const fetchScript = (scriptWindow, script) => {
  return $networkUtils
    .fetch(script.relativeUrl, scriptWindow)
    .then((contents) => {
...
//   return $sourceMapUtils
//     .initializeSourceMapConsumer(script, sourceMap)
//     .return([script, contents])
// }
...
const runScripts = (specWindow, scripts) => {
  return Promise.map(scripts, (script) => fetchScript(specWindow, script))
