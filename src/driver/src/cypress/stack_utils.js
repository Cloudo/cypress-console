...
const stackWithUserInvocationStackSpliced = (err, userInvocationStack) => {
  ...
  const [messageLines, stackLines] = splitStack(stack)
  const userInvocationStackWithoutMessage = stackWithoutMessage(
    userInvocationStack
  )
...
const getCodeFrameFromSource = (
  sourceCode,
  { line, column, relativeFile, absoluteFile }
) => {
...
const getCodeFrame = (err, stackIndex) => {
  ...
  return getCodeFrameFromSource(
    $sourceMapUtils.getSourceContents(fileUrl, originalFile),
    stackLine
  )
}
...
const getSourceDetails = (generatedDetails) => {
  const sourceDetails = $sourceMapUtils.getSourcePosition(
    generatedDetails.file,
    generatedDetails
  )
...
const getSourceStack = (stack, projectRoot) => {
  ...
  const getSourceDetailsWithStackUtil = _.partial(
