...
const isNeedSingleValueChangeInputElement = (
  ...
  return inputTypeNeedSingleValueChangeRe.test(
    (el.getAttribute('type') || '').toLocaleLowerCase()
  )
}
...
const isContentEditable = (
  el: HTMLElement
): el is HTMLContentEditableElement => {
  return (
    getNativeProp(el, 'isContentEditable') ||
    $document.getDocumentFromElement(el).designMode === 'on'
  )
}
...
const isFocusedOrInFocused = (el: HTMLElement) => {
  ...
  if (
    elToCheckCurrentlyFocused &&
    elToCheckCurrentlyFocused === activeElement
  ) {
    if (isContentEditableEl) {
      ...
      if (sel?.rangeCount) {
        ...
        debug(
          'isInFocused by document selection?',
          selectionInsideElement,
          ':',
...
const isW3CRendered = (el) => {
  ...
  return !(
    parentHasDisplayNone(wrap(el)) || wrap(el).css('visibility') === 'hidden'
  )
}
