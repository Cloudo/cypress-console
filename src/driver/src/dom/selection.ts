...
const _getSelectionBoundsFromContentEditable = function (el) {
  ...
  const hostContenteditable = getHostContenteditable(
...
const _hasContenteditableAttr = (el) => {
  const attr = $elements.tryCallNativeMethod(
    el,
    'getAttribute',
    'contenteditable'
  )
...
const _moveCursorUpOrDown = function (up: boolean, el: HTMLElement) {
  ...
  if (isTextarea && Cypress.browser.family === 'firefox') {
    ...
    if (up) {
      ...
      toPos =
        baseLen + (bounds.end - val.slice(0, bounds.end).lastIndexOf('\n'))
    }
...
const _moveCursorToLineStartOrEnd = function (
  toStart: boolean,
  el: HTMLElement
) {
  ...
  if ($elements.isContentEditable(el) || isInputOrTextArea) {
    ...
    if (Cypress.browser.family === 'firefox' && isInputOrTextArea) {
      if (isInput) {
        ...
        if (!toStart) {
          toPos = $elements.getNativeProp(el as HTMLInputElement, 'value')
            .length
        }
      ...
      if (isTextarea) {
        ...
        const value = $elements.getNativeProp(
...
const _moveSelectionTo = function (
  toStart: boolean,
  el: HTMLElement,
  options = {}
) {
...
const replaceSelectionContents = function (el, key) {
  ...
  if ($elements.isInput(el) || $elements.isTextarea(el)) {
    ...
    debug(
      `inserting at selection ${JSON.stringify({ start, end })}`,
      'rewriting value to ',
      updatedValue
    )
